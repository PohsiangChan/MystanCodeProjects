...
def get_average(pixels):
    ...
    #     green += pixel.green
    #     blue += pixel.blue
    # color_list = [int(red / len(pixels)), int(green / len(pixels)), int(blue / len(pixels))]
    # return color_list

    # python comprehension，用 // 就會變成整數
...
def get_best_pixel(pixels):
    ...
    dist_list = []
    for pixel in pixels:
        # if minimal_dist > get_pixel_dist(pixel, pixel.red, pixel.green, pixel.blue):
        #     minimal_dist = get_pixel_dist(pixel, pixel.red, pixel.green, pixel.blue)
        #     best_pixel = pixel
        # 會變成自己跟自己比較，而非 avg，導致算出來結果為第一張
...
def solve(images):
    ...
    for i in range(width):
        for j in range(height):
            pixels = []
            for img in images:
                # 取得 N 張圖片在 (i, j) 的 pixel，並將他製作成一張 list
