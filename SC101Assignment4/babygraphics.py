...
def get_x_coordinate(width, year_index):
    ...
    if year_index == 0:
        ...
        x_coordinate = GRAPH_MARGIN_SIZE + year_index * ((width - GRAPH_MARGIN_SIZE*2 - len(YEARS)*2) / (len(YEARS)-1))
    return x_coordinate
...
def draw_fixed_lines(canvas):
    ...
    # space = (CANVAS_WIDTH - GRAPH_MARGIN_SIZE*2 - len(YEARS)*2) / (len(YEARS)-1)
...
def draw_names(canvas, name_data, lookup_names):
    ...
    # ----- Write your code below this line ----- #
    unit_length = (CANVAS_HEIGHT - GRAPH_MARGIN_SIZE*2) / MAX_RANK

    for j in range(len(lookup_names)):
        name = lookup_names[j]
        for i in range(len(YEARS)-1):
            if str(YEARS[i]) in name_data[name]:
                y1 = int(name_data[name][str(YEARS[i])])*unit_length+GRAPH_MARGIN_SIZE
                tag1 = (name + ' ' + str(name_data[name][str(YEARS[i])]))
            else:
                y1 = CANVAS_HEIGHT-GRAPH_MARGIN_SIZE
                tag1 = (name + ' *')

            if str(YEARS[i+1]) in name_data[name]:
                y2 = int(name_data[name][str(YEARS[i+1])])*unit_length+GRAPH_MARGIN_SIZE
                tag2 = (name + ' ' + str(name_data[name][str(YEARS[i+1])]))
            else:
                y2 = CANVAS_HEIGHT-GRAPH_MARGIN_SIZE
                tag2 = (name + ' *')

            canvas.create_line(get_x_coordinate(CANVAS_WIDTH, i), y1,
                               get_x_coordinate(CANVAS_WIDTH, i+1), y2, fill=COLORS[j % len(COLORS)])
            canvas.create_text(get_x_coordinate(CANVAS_WIDTH, i)+TEXT_DX, y1,
                               text=tag1, anchor=tkinter.SW, fill=COLORS[j % len(COLORS)])  # anchor 為基準點位於文字的相對位置
            canvas.create_text(get_x_coordinate(CANVAS_WIDTH, i+1)+TEXT_DX, y2,
                               text=tag2, anchor=tkinter.SW, fill=COLORS[j % len(COLORS)])
